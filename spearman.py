...
class canvas(object):
    ...
    def xlabel(self,label,axis=None,fontsize=22):
        a = self.set_axis(axis)
        a.set_xlabel(label,fontsize=fontsize)

    def ylabel(self,label,axis=None,fontsize=22):
        a = self.set_axis(axis)
    ...
    def legend(self,axis=None,loc='lower right',ncol=1):
        a = self.set_axis(axis)
        ...
        if loc == 'above':
            ...
            a.legend(loc='center left', bbox_to_anchor=(1, 0.5),fancybox=True)

        else:
            a.legend(loc=loc,fancybox=True)
...
def get_moving_spearman_rank(xs,ys,colours,
    ...
    if isinstance(window_sizes,list):
        ...
        for i in range(0, len(window_sizes)): # Need to 
    ...
    print(starts)
    print(stops)
    ys[starts[r]:stops[r]],colours[starts[r]:stops[r]]

    # Step through the bins
    for r in range(0, len(moving_rank)):
